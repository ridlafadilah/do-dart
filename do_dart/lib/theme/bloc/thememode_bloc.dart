...
class ThemeModeBloc extends Bloc<ThemeModeEvent, ThemeModeState> {
  ThemeModeBloc() : super(const ThemeModeState(themeMode: ThemeMode.light));
  ...
  Stream<ThemeModeState> mapEventToState(ThemeModeEvent event) async* {
    ...
    if (darkMode) {
      yield const ThemeModeState(themeMode: ThemeMode.dark);
    } else {
      yield const ThemeModeState(themeMode: ThemeMode.light);
    }
  }
}
